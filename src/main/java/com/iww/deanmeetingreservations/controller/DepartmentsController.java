...
public class DepartmentsController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<List<DepartmentDeanDutiesDto>> getDeansWithOfficeHoursForGivenDepartment(@PathVariable("id") UUID id) {
        try {
            return new ResponseEntity<>(departmentsService.getDeansWithOfficeHours(id), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
